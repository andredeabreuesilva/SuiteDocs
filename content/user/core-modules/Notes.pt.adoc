...
O módulo Notes no SuiteCRM pode ser usado para manter um registro de qualquer
comentários, observações ou explicações que um Usuário possa ter relacionado
internamente à organização ou em relação a outro registro do SuiteCRM
como uma conta, contato, lead ou muito mais. As notas também são usadas para
manter um registro das interações com os clientes em relação a Casos e Bugs.

== Ações de Notas

Você pode acessar as Ações de Notas no menu suspenso do módulo Notas
para baixo ou pela barra lateral. As ações de Notas são as seguintes:

* *Criar Notas* - Um novo formulário é aberto na Visualização de edição para permitir que você crie um novo registro de Notas.
* *Exibir Notas* - Redireciona você para a exibição em lista dos registros do módulo Notas. Isso permite pesquisar e listar registros de Notas.
* *Importar Notas* - Redireciona você para o Assistente de importação de Notas. Para mais informações, veja link:./../../introduction/user-interface/record-management/#_importing_records[Importing Records].

Para visualizar a lista completa de campos disponíveis ao criar um registro de Notas, veja
